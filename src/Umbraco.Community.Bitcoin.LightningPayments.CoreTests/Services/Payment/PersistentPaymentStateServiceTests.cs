...
{
    /// <summary>
    /// Unit tests for PersistentPaymentStateService.
    /// </summary>
    public class PersistentPaymentStateServiceTests
    {
        private PersistentPaymentStateService CreateService()
        {
            ...
            return new PersistentPaymentStateService(context);
        }
        ...
        [Fact]
        public async Task AddPendingPaymentAsync_WithTipContent_AddsPayment()
        {
            // Arrange
            var service = CreateService();
            var paymentHash = "hash123";
        ...
        [Fact]
        public async Task ConfirmPaymentAsync_WithIdempotencyMapping_UpdatesMappingStatus()
        {
            // Arrange
            var service = CreateService();
            var paymentHash = "hash123";
            ...
            await service.AddPendingPaymentAsync(paymentHash, contentId, userSessionId);
            await service.TryCreateMappingAsync(idempotencyKey, paymentHash, invoice);
            ...
            // Assert
            Assert.Equal(PaymentConfirmationResult.Confirmed, result);
            var mapping = await service.TryGetMappingByKeyAsync(idempotencyKey);
            Assert.Equal(PaymentStatus.Paid, mapping?.Status);
        }
