...
public class LightningPaymentsSettingsValidator : IValidateOptions<LightningPaymentsSettings>
{
    ...
    public ValidateOptionsResult Validate(string? name, LightningPaymentsSettings options)
    {
        ...
        try
        {
            isValid = Validator.TryValidateObject(options, context, validationResults, true);
        }
        catch (Exception ex)
        {
            _logger.LogCritical(ex, "Validation threw an exception");
        ...
        if (_env.IsProduction())
        {
            ...
            if (!string.IsNullOrWhiteSpace(options.BreezApiKey) && !HasEnv("LightningPayments__BreezApiKey"))
                leaked.Add("BreezApiKey");
            if (!string.IsNullOrWhiteSpace(options.Mnemonic) && !HasEnv("LightningPayments__Mnemonic"))
                leaked.Add("Mnemonic");
            if (!string.IsNullOrWhiteSpace(options.WebhookSecret) && !HasEnv("LightningPayments__WebhookSecret"))
                leaked.Add("WebhookSecret");
