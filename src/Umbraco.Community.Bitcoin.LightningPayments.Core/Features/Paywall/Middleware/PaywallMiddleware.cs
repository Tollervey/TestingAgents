...
namespace Umbraco.Community.Bitcoin.LightningPayments.Core.Features.Paywall.Middleware
{
    /// <summary>
    ...
    public class PaywallMiddleware
    {
            ...
            IPaymentStateService paymentStateService)
        {
            ...
            //2a) Donï¿½t interfere with editor preview
            ...
            {
                // On bad JSON, donï¿½t block the page
                await _next(context);
                return;
            }
            ...
            //5) If we have a session cookie and itï¿½s paid for this content, allow through
        ...
        private static bool ShouldBypass(HttpContext context)
        {
            ...
            // Skip for ALL /umbraco paths (backoffice, API, Swagger, management, etc.)
            if (path.StartsWithSegments(BackofficePrefix, StringComparison.OrdinalIgnoreCase))
                return true;

            // Skip for /api paths (public APIs)
            if (path.StartsWithSegments(new PathString("/api"), StringComparison.OrdinalIgnoreCase))
                return true;
