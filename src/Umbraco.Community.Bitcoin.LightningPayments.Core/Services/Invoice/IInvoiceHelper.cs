...
using Umbraco.Community.Bitcoin.LightningPayments.Core.Features.Paywall.Models;
...
namespace Umbraco.Community.Bitcoin.LightningPayments.Core.Services.Invoice;
...
internal sealed class InvoiceHelper : IInvoiceHelper
{
    ...
    public async Task<(string invoice, string paymentHash)> CreateInvoiceAndHashAsync(ulong amountSat, string description, string? idempotencyKey = null)
    {
        ...
        // Not found or no idempotency key supplied ï¿½ create a new invoice via Breez SDK
        var invoice = await _breezSdkService.CreateInvoiceAsync(amountSat, description);
        var hash = await TryGetPaymentHashAsync(invoice);
        if (string.IsNullOrWhiteSpace(hash))
            throw new InvalidInvoiceRequestException("Failed to obtain invoice payment hash.");
    ...
    public string EnsureSessionCookie(HttpRequest request, HttpResponse response, string? explicitState = null)
    {
        ...
        ? explicitState!
        : (request.Cookies[PaywallMiddleware.PaywallCookieName] ?? Guid.NewGuid().ToString());
        var options = new CookieOptions { HttpOnly = true, Secure = true, SameSite = _settings.SessionCookieSameSite };
        if (!string.IsNullOrWhiteSpace(_settings.SessionCookieDomain))
        { options.Domain = _settings.SessionCookieDomain; }
        response.Cookies.Append(PaywallMiddleware.PaywallCookieName, sessionId, options);
        return sessionId;
    }
