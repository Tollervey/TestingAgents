...
namespace Umbraco.Community.Bitcoin.LightningPayments.Core.Services.Breez
{
    ...
    /// It returns a synthetic "invoice" string that embeds a global::Breez.Sdk.Liquid.Payment hash and confirms it after a configurable delay.
    ...
    internal sealed class OfflineBreezSdkService : IBreezSdkService, IBreezSdkHandleProvider
    {
        ...
        {
            _logger = logger;
            _settings = settings?.Value ?? new LightningPaymentsSettings();
        ...
        // New: quote expected fees for receiving a global::Breez.Sdk.Liquid.Payment before creating an invoice/offer (offline ->0)
        ...
        public Task<global::Breez.Sdk.Liquid.RecommendedFees?> GetRecommendedFeesAsync(CancellationToken ct = default)
            => Task.FromResult<global::Breez.Sdk.Liquid.RecommendedFees?>(null);
        ...
        public Task<global::Breez.Sdk.Liquid.Payment?> GetPaymentByHashAsync(string paymentHash, CancellationToken ct = default)
            => Task.FromResult<global::Breez.Sdk.Liquid.Payment?>(null);
        ...
        public Task<global::Breez.Sdk.Liquid.BindingLiquidSdk?> GetSdkAsync(CancellationToken ct = default)
            => Task.FromResult<global::Breez.Sdk.Liquid.BindingLiquidSdk?>(null);
        ...
        private async Task SimulateConfirmationAsync(string paymentHash, CancellationToken token)
        {
            try
            {
                ...
                _logger.LogInformation("Offline mode: global::Breez.Sdk.Liquid.Payment confirmed for hash {PaymentHash}", paymentHash);
            }
        ...
        public Task<List<global::Breez.Sdk.Liquid.Payment>> GetPaymentsAsync(CancellationToken ct = default)
        {
            // Offline mode: no real payments to retrieve, return empty list
            return Task.FromResult(new List<global::Breez.Sdk.Liquid.Payment>());
        }
    }
}
