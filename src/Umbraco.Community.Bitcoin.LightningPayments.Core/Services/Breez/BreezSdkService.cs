...
namespace Umbraco.Community.Bitcoin.LightningPayments.Core.Services.Breez
{
    ...
    /// - This service exposes high-level methods to create Lightning global::Breez.Sdk.Liquid.Payment requests (BOLT11/BOLT12) and
    ...
    public class BreezSdkService : IBreezSdkService, IBreezSdkHandleProvider, IAsyncDisposable
    {
        ...
        public BreezSdkService(IOptions<LightningPaymentsSettings> settings, IHostEnvironment hostEnvironment, ILoggerFactory loggerFactory, ILogger<BreezSdkService> logger, IBreezSdkWrapper wrapper, IServiceProvider serviceProvider)
        {
            ...
            var receiveRetry = Policy<ReceivePaymentResponse>.Handle<TimeoutException>().Or<HttpRequestException>().Or<SocketException>()
                .WaitAndRetryAsync(1, _ => TimeSpan.FromSeconds(2),
                onRetryAsync: (outcome, ts, attempt, ctx) => { _logger.LogWarning("Retry {Attempt} for receive global::Breez.Sdk.Liquid.Payment after {TimeSpan.TotalSeconds}s due to {Message}", attempt, ts, outcome.Exception?.Message ?? "unknown error"); return Task.CompletedTask; });
        ...
        private async Task<BindingLiquidSdk?> InitializeSdkAsync(CancellationToken ct = default)
        {
            ...
            try
            {
                ...
                config = config with { workingDir = workingDir };
        ...
        private async Task<string> CreatePaymentAsync(ulong amountSat, string description, PaymentMethod paymentMethod, string paymentType, CancellationToken ct)
        {
            ...
            catch (Exception ex) when (ex is not InvalidInvoiceRequestException && ex is not OperationCanceledException)
            {
                activity?.SetStatus(ActivityStatusCode.Error, ex.Message);
        ...
        /// <summary>
        /// Creates a Lightning global::Breez.Sdk.Liquid.Payment request using the BOLT11 standard (one-off invoice with amount and optional description).
        ///
        ...
        /// <summary>
        /// Creates a Lightning global::Breez.Sdk.Liquid.Payment request using BOLT12 offer (Liquid implementation only).
        ///
        /// UI guidance (Receive UX):
        ...
        /// <summary>
        /// Attempts to parse a BOLT11 invoice to extract its global::Breez.Sdk.Liquid.Payment hash using the SDK's <c>parse</c> facility.
        ///
        /// UI guidance (Display UX):
        /// - Use the global::Breez.Sdk.Liquid.Payment hash to correlate local UI state with on-chain/Lightning global::Breez.Sdk.Liquid.Payment entities.
        ...
        /// <summary>
        /// Retrieves a single global::Breez.Sdk.Liquid.Payment by its Lightning global::Breez.Sdk.Liquid.Payment hash.
        ///
        /// UI guidance (Display UX):
        /// - Use this to populate a global::Breez.Sdk.Liquid.Payment details screen. Display amount and fees separately, and expose invoice/preimage
        /// in a Details section. Represent current state (Pending/Succeeded/Failed) with distinct visuals.
        ///
        /// References:
        /// - https://sdk-doc-liquid.breez.technology/guide/uxguide_display.html
        /// </summary>
        public async Task<global::Breez.Sdk.Liquid.Payment?> GetPaymentByHashAsync(string paymentHash, CancellationToken ct = default)
        {
            var sdk = await _sdkInstance.Value.WaitAsync(ct);
            if (sdk == null)
            {
        ...
        /// <summary>
        /// Retrieves a list of payments from the Breez SDK for history/display purposes.
        ///
        /// UI guidance (Display UX):
        /// - Use this to populate a payments history screen. Display amount, fees, status, and timestamps.
        /// - Paginate if the list is large; consider filtering by status or date.
        ///
        /// References:
        /// - https://sdk-doc-liquid.breez.technology/guide/uxguide_display.html
        /// </summary>
        public async Task<List<global::Breez.Sdk.Liquid.Payment>> GetPaymentsAsync(CancellationToken ct = default)
        {
            var sdk = await _sdkInstance.Value.WaitAsync(ct);
            if (sdk == null)
            {
            ...
            return await _wrapper.ListPaymentsAsync(sdk, req, ct);
        }

        /// <summary>
        /// Fetches a quote for the fees required to receive a global::Breez.Sdk.Liquid.Payment of the specified amount.
        /// Uses the existing prepare policy to estimate the fees without creating a global::Breez.Sdk.Liquid.Payment request.
        /// </summary>
        ...
        internal void ValidateInvoiceDescription(string description)
        {
            ...
            // New: explicitly reject any control characters (ASCII < 32 or 127 DEL). This catches null (\x00) and other non-printables
            // rather than relying solely on the regex which may treat embedded null as end-of-string in certain runtimes.
            foreach (var ch in description)
            {
                if (char.IsControl(ch))
                {
                    _logger.LogWarning("Invoice description contains control character U+{CodePoint:X4}.", (int)ch);
        ...
        ///
        /// UI guidance:
        /// - Use this to show/hide a countdown timer for global::Breez.Sdk.Liquid.Payment expiry in the UI.
        ...
        public async Task<DateTimeOffset?> TryExtractInvoiceExpiryAsync(string invoice, CancellationToken ct = default)
        {
            ...
            try
            {
                ...
                if (parsed is InputType.Bolt11 bolt11)
                {
                    ...
                    // Expiry is ttl in seconds from creation timestamp
                    var createdUnix = inv.timestamp;
                    var ttl = inv.expiry;
                    // Validate that both values are positive and within reasonable Unix timestamp bounds
                    // to avoid overflow issues from wrapped negative values
                    const ulong maxReasonableTimestamp = long.MaxValue; // Max value that can be safely converted to long
                    if (createdUnix > 0 && ttl > 0 &&
                        createdUnix < maxReasonableTimestamp &&
                        ttl < maxReasonableTimestamp &&
                        createdUnix <= (maxReasonableTimestamp - ttl)) // Ensure no overflow when adding
                    {
                        return DateTimeOffset.FromUnixTimeSeconds((long)(createdUnix + ttl));
                    }
                }
            }
            catch (Exception ex)
