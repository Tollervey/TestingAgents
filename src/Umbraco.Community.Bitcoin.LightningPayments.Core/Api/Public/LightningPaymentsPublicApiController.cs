...
public class LightningPaymentsPublicApiController : ControllerBase
{
    ...
    public async Task<IActionResult> GetPaywallInvoice([FromQuery] int contentId)
    {
        ...
        if (!rate.Allowed)
        { Response.Headers["Retry-After"] = Math.Ceiling(rate.RetryAfter.TotalSeconds).ToString(); return Error(StatusCodes.Status429TooManyRequests, "rate_limited", "Too many requests. Please try again shortly."); }
        if (contentId <= 0)
    ...
    public async Task<IActionResult> GetLnurlInvoice([FromQuery] long amount, [FromQuery] int contentId, [FromQuery] string? state = null)
    {
        ...
        if (!rate.Allowed)
        { Response.Headers["Retry-After"] = Math.Ceiling(rate.RetryAfter.TotalSeconds).ToString(); return StatusCode(StatusCodes.Status429TooManyRequests, "Too many requests. Please try again shortly."); }
        if (contentId <= 0)
    ...
    public async Task<IActionResult> GetPaymentStatusByHash([FromQuery] string paymentHash)
    {
        ...
        { return BadRequest(new { error = "invalid_request", message = "Missing paymentHash." }); }
    ...
    public async Task<IActionResult> GetPaywallReceiveFeeQuote([FromQuery] int contentId, [FromQuery] bool bolt12 = false)
    {
        ...
        {
            _logger.LogWarning(ex, "Invalid request for public fee quote.");
            return Error(StatusCodes.Status400BadRequest, "invalid_request", ex.Message);
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Error fetching public receive fee quote for contentId {ContentId}", contentId);
